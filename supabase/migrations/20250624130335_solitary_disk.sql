/*
  ...
  2. Calculations
    - Engagement rate calculated as (likes + shares + comments_count) / NULLIF(views, 0) * 100
    - Click-through rate calculated as total_clicks / NULLIF(total_views, 0) * 100
    - Conversion rate calculated as unique_clicks / NULLIF(unique_views, 0) * 100

  3. Security
    - Functions are accessible to authenticated and anonymous users for tracking
    - Admin-only functions for sensitive analytics data
*/
      ...
      DO UPDATE SET 
        ...
        unique_views = post_analytics.unique_views + 
          CASE WHEN NOT EXISTS (
            ...
            AND ee.id != (SELECT currval('engagement_events_id_seq'))
          ) THEN 1 ELSE 0 END,
        last_viewed = now(),
        updated_at = now();
